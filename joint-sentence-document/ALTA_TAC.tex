...
%Understanding party position is one of the primary objectives of manifesto text analysis.
...
% SS; We use segmentations given by annotators, it will not have any ambiguity right? (not sure if I understood it fully, but the test set we use is annotated, and all annotated documents are segmented also)
...
\end{equation}
where $R$ and $L$ denote right and left political themes (see Figure 5), 
%where, $R = \{$104, 201, 203, 305, 401, 402, 407, 414, 505, 601, 603, 605, 606$\}$ and $L = \{$103, 105, 106, 107, 202, 403, 404, 406, 412, 413, 504, 506, 701$\}$
and $\per_{t}$ denotes the share of each topic $t$ as given in \tabref{fig:CMP}, per document.  Note that the RILE score is provided for almost all the manifestos in the CMP dataset, but the sentence-level annotations are provided only for a subset of manifestos. That is, in some cases, the underlying annotations that the RILE score calculation was based on is often not available for a given manifesto.
...
\begin{figure*}[!t]
\centering
\includegraphics[height=8.4cm, scale=1.5]{ALTA_Model_22.pdf}
\caption{Hierarchical Neural Network for Joint Sentence--Document Analysis. $s_{1}$, $s_{2}$,...$s_{n}$ are input sentences ($n$=$l_{d}$), $W_{s}$ and $W_{p}$ are shared across unrolled sentences. $\hat{\mathbf{y}}_{{ij}}$ denotes  57 classes and $\hat{z}_{i}$ denotes the estimated RILE score}
 \label{fig:HNN}
 \end{figure*}

\subsection{Sentence-level Model}
We represent each sentence using the average embedding of its constituent words, $ \bm{s}_{j} = \frac{1}{|s_{j}|}\sum_{w \in s_{j}} e_{w}$.
...
%\frac{1}{\sum_{i=1}^{D_{s}}l_{d_{i}}}
  \begin{equation}
    \label{eq:sent-loss}
    \mathcal{L}_{S} = 
    -\frac{1}{|D_{s}|} \sum_{i=1}^{|D_{s}|}\sum_{j=1}^{l_{d}}\sum_{k=1}^{K} y_{{ijk}} \log \hat{y}_{{ijk}}  
  \end{equation}
%\footnote{\url{https://keras.io/layers/wrappers/\#timedistributed}%}

\subsection{Joint Sentence--Document Model}
Using the hierarchical neural network, we model the sentence-level classification and document-level regression tasks together. In the joint model, we use an unrolled (time-distributed) neural network model for the sentences in a manifesto ($d$). Here, the model minimizes cross-entropy loss for sentences over each temporal layer ($j=1 \ldots l_{d}$). We use average-pooling with the concatenated hidden representations ($\mathbf{h}_{ij}$) and predicted output distributions ($\hat{\mathbf{y}}_{ij}$) of individual sentences, to represent a document,\footnote{We observed that the concatenated representation performed better than using either hidden representation or output distribution.} i.e.,
$ \mathbf{r}_d = \frac{1}{|l_{d}|}\sum_{j \in d} \left[\begin{array}{cc} \hat{\mathbf{y}}_{ij} \\ \mathbf{h}_{ij} \end{array} \right]  $.

The range of RILE is $[-100,100]$, which we scale to the range $[-1,1]$. Hence we use a final $\tanh$ layer, with $\hat{z}_i = \mathbf{w}_{r}^\top \mathbf{h}_{d} + b$, where $\mathbf{h}_{d} = \text{ReLU}(W_{d}^\top \mathbf{r}_d)$.
...
%\frac{1}{|D|}  (\hat{Y}_{d} - Y_{d})^\top (\hat{Y}_{d} - Y_{d}).
\label{eq:doc-loss}
\end{equation}
...
%$\mathcal{L}_S(\hat{Y},Y)$ 
%$\mathcal{L}_D(\hat{Z}, Z)$
\begin{description}
...
\midrule
 Total    & 948 (235)  & 489858 (180931)\\
  ...
  \end{tabular}
  \caption{Statistics of dataset, `Ann.' refers to annotated at sentence level.}
  \label{tab:al}
\end{table}
...
\centering
  ...
  \caption{Class distribution based on average percentage of sentences coded under each class}
  \label{fig:cd}
\end{figure}
...
%, since we found it empirically better compared to other approaches
%Following are the sentence classification baseline techniques that we compare our proposed approach against.
%, which is the state-of-art approach for fine-grained manifesto sentence classification 
\begin{description}
...
%(7 major domains as shown in \tabref{fig:CMP})
\begin{description}
...
Commonly for all empirical evaluations, we compute micro-averaged performance with 80-20\% train-test ratio across 10 runs with random split (at document level), where the 80\% split also contains sentence level annotated documents proportionally. Optimal model parameters we found for the proposed model (Figure \ref{fig:HNN}) are $\norm{\mathbf{h}_{ij}}$ = 300 (for sentences), $\norm{\mathbf{h}_{d}}$ = 10. We compute F-score\footnote{Harmonic mean of precision and recall, \url{https://en.wikipedia.org/wiki/F1_score}} to evaluate sentence classification performance. Sentence classification performance is given in \tabref{tab:alx}. Under mono-lingual setting (\tabref{tab:alx}), using word embeddings did not provide better performance compared to bag-of-words. 
...
%\rm It must be considered that the (sentence and document-level) model built using cascaded training is a special case of jointly trained model, with suitable values of $\alpha$ (closer to 1). 
%Also \em{CNN}, \em{AE-NN}$_{c}$ and \em{JT}$_{s}$ \rm use combined training instances across languages compared to other approaches.
%Except for $da$ and $it$ cross-lingual training performs better than mono-lingual setting.
...
\[ \text{cluster} (w) = \argmin_k \text{dist}(C_{k}, w). \]
...
\centering
\includegraphics[width=0.9\linewidth]{altw1.pdf}
...
\begin{subfigure}{0.45\textwidth}
\centering
\includegraphics[width=0.9\linewidth]{altw2.pdf}
\caption{Fixing 80\% training documents with sentence-level  annotations, ratio of documents with RILE score is varied. }
\label{fig:docl}
\end{subfigure}
\label{annotation}
\caption{Study with Quantity of Annotation. In 3(a) and 3(b) \begin{small} cross($\markera$) \end{small} denotes \em{Cas}$_{d}$ \rm and \begin{small} circle($\markerb$)\end{small} denotes \em{JT}$_{d}$}
\end{figure}
...
%         \centering
%         \includegraphics[height=1in]{altw1.pdf}
%         \caption{Lorem ipsum}
%     \end{subfigure}%
%     ~ 
%     \begin{subfigure}[t]{0.6\textwidth}
%         \centering
%         \includegraphics[height=1in]{altw2.pdf}
...
Since the definition of left--right varies between countries, we study the influence of \textit{country} information in the proposed model with \textit{joint-training}. We use two ways to incorporate country information \cite{hoang2016incorporating}: (a) \textit{stack} --- one-hot encoding (13 countries, 1 $\times$ 13 vector) of each manifesto's \textit{country} is concatenated with hidden representation of the document ($\mathbf{r}_{d}$ in Figure \ref{fig:HNN}) (b) \textit{non-linear stack} --- one-hot-encoded country vector is passed through a hidden layer with $\tanh$ non-linear activation and concatenated with $\mathbf{r}_{d}$. With both the models we observed mild improvement in correlation (given in \tabref{tab:al2}).
...
\caption{Manifesto snippet for Democratic Party of USA, 2000 --- $\int$ denotes sentence segment. See Table 5 for code description.}
\label{fig:sub2}
\end{figure*}
