...
\end{equation}
where $0 \le \alpha \le 1$\footnote{We use $\alpha$=0.5, since we give equal weight for sentence and document level tasks. We observed a tradeoff in performance with different $\alpha$, with lesser $\alpha$ (0.1), document level correlation increases with decrease in sentence level F-measure. Higher value of $\alpha$ (0.9) gives performance closer to cascaded training}.
