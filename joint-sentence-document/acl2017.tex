...
\begin{figure*}[!ht]
\centering
\includegraphics[height=8.3cm, scale=1.3]{Joint_Model_N8.png}
...
\begin{equation}
 \alpha * L_{s}(D_{s},Y_{s}) + (1-\alpha) * L_{d}(\hat{Y}_{d}, Y_{d})\footnote{We use $\alpha$=0.5, since we give equal weight for sentence and document level tasks. We observed a tradeoff in performance with different $\alpha$, with lesser $\alpha$ (0.1), document level correlation increases upto 0.52, with sentence level F-measure around 0.33. Higher value of $\alpha$ (0.9) gives performance closer to cascaded training} 
\end{equation}
